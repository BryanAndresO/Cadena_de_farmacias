...
public class SecurityConfig {
    ...
    /**
     * GatewayFilter to intercept /logout requests BEFORE they get routed to frontend.
            ...
            if ("/logout".equals(path)) {
                ...
                return exchange.getPrincipal()
                    ...
                    .switchIfEmpty(Mono.defer(() -> {
                        ...
                        exchange.getResponse().getHeaders().setLocation(URI.create("/oauth2/authorization/gateway-client"));
    ...
    @Bean
        ...
        ServerLogoutSuccessHandler logoutSuccessHandler = (exchange, authentication) -> {
            // Try to perform an OP (IdP) logout using end_session_endpoint and id_token_hint.
            // If we cannot resolve provider logout URL or id token, fall back to redirecting to '/'.
            ...
            if (authentication instanceof OAuth2AuthenticationToken) {
                ...
                return clientRegistrationRepository.findByRegistrationId(registrationId)
                    .flatMap(reg -> {
                        // Try to get end_session_endpoint from metadata, otherwise construct it from authorization-uri
                        Object endSessionObj = reg.getProviderDetails().getConfigurationMetadata().get("end_session_endpoint");
                        ...
                        if (endSessionObj != null) {
                            ...
                            // e.g., http://localhost:9000/oauth2/authorize -> http://localhost:9000/connect/logout
                    ...
                    .switchIfEmpty(Mono.defer(() -> {
                        ...
                        exchange.getExchange().getResponse().setStatusCode(HttpStatus.FOUND);
                        exchange.getExchange().getResponse().getHeaders().setLocation(URI.create("/oauth2/authorization/gateway-client"));
                        return Mono.<Void>empty();
                    }));
            }
        ...
        http
            .authorizeExchange(exchanges -> exchanges
                ...
                .pathMatchers("/", "/index.html", "/assets/**", "/static/**", "/src/**", "/*.js", "/*.css", "/*.svg", "/*.ico").permitAll()
                // Allow public API endpoints only when NOT authenticated - this ensures userinfo returns unauthenticated after logout
